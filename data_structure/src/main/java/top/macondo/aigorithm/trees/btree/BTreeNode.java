...
public class BTreeNode {
    ...
    public BTreeNode insert(int e) {
        ...
        if(isEmpty()) {
            values.add(e);
            children.add(new BTreeNode(this));//e关键字左侧
            children.add(new BTreeNode(this));//e关键字右侧
            return this;
        }
        //从根节点开始找要插入的节点
    ...
    /**
     * 删除指定节点中的关键字和孩子，并处理删除后打破B-树规则的情况 (这里删除的，其实都是叶子节点）
     * @param target 目标节点
     ...
     *
     * @param target
     * @return 找到则返回找到的节点，不存在则返回叶子节点（关键值为空的节点）
     */
    public BTreeNode search(int target) {
        ...
        /**
         * 遍历当前节点 关键字<=目标值。等于时直接返回当前节点，小于且刚好不大于时，继续找节点的右孩子节点
         * 即，从左往右遍历关键字，遇到大于的，下移到子节点继续找。找到那个有节点等于目标值，或者遍历到叶子节点
         */
        int valueIndex = 0;
